...
- **Статистика** считает задачи, реально находящиеся в днях диапазona (выполнено/невыполнено), плюс отдельно — сколько задач было перенесено из дней этого диапазона, с разбивкой по категориям.
